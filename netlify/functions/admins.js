     ...
     if (method === 'POST') {
       ...
       if (body.medico_id) {
         ...
         if (medErr) throw medErr;
          if (!med.email) return { statusCode: 400, headers, body: JSON.stringify({ error: 'El médico no tiene correo registrado' }) };
          const { data: exists, error: existsErr } = await supabase.from('admins').select('id').eq('medico_id', body.medico_id).maybeSingle();
          if (existsErr) throw existsErr;
          if (exists) return { statusCode: 409, headers, body: JSON.stringify({ error: 'Este médico ya tiene una cuenta de acceso' }) };
          const { data: emailExists } = await supabase.from('admins').select('id').eq('email', med.email.toLowerCase()).maybeSingle();
          if (emailExists) return { statusCode: 409, headers, body: JSON.stringify({ error: 'Ya existe un admin con ese correo' }) };
