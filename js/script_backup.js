...
async function syncFromApi() {
    ...
    const [pacientes, medicos, citas, adminsApi] = await Promise.all([
        apiGetPacientes(), apiGetMedicos(), apiGetCitas(), apiGetAdmins().catch(() => [])
    ]);
...
function editPatient(id) {
    ...
    // Llenar ciudad/población del paciente (después de cargar municipios)
    document.getElementById('editPatCity').value = patient.city;
    // Cargar localidades con el city del paciente
    loadEditLocalidades(stateValue, patient.city);
    // Después de cargar localidades, establecer el valor de población si existe
    // Esto asegura que el valor se establezca después de que el select sea poblado
    setTimeout(() => {
        const popSelect = document.getElementById('editPatPopulation');
        if (popSelect && patient.population) {
            popSelect.value = patient.population;
        }
    }, 10);
...
document.getElementById('stepNext1').addEventListener('click', () => {
    ...
    // Cargar municipios después de nombre/género
...
function deletePatient(id) {
    if (!requireAdminAuth()) return;
    if (!confirm('⚠️ ¿Eliminar paciente y todas sus citas?')) return;
    const index = patients.findIndex(p => p.id === id);
    if (index === -1) return;
    patients.splice(index, 1);
    const removedAppointments = appointments.filter(app => app.patientId === id).map(a => a.id);
    appointments = appointments.filter(app => app.patientId !== id);
    saveData();
    renderDoctorAgenda();
    // Limpiar selección si la cita seleccionada fue eliminada
    // Limpiar selección en localStorage/admins si alguna cita del paciente estaba seleccionada
    for (const remId of removedAppointments) {
        clearSelectedAppointmentIfMatchesId(remId);
    }
    updatePatientSelect();
    renderPatientList();
    renderAdminPatientList();
    renderAgenda();
    showMessage(document.getElementById('patientMsg'), 'Paciente eliminado junto con sus citas.');
}
...
function loadEditLocalidades(estado, municipio) {
    ...
    // Si no encuentra el municipality en mexicoData, usar fallback:
    // intentar con cualquier municipality que tenga localidades
    if (!foundMunicipio) {
...
function loadDemoData() {
    if (patients.length === 0) {
        const demoPatients = [
            { id: 1, name: "Ana Cristina Fernández Pérez", curp: "FEPA921108MNTRN03", dob: "1992-11-08", gender: "Femenino", phone: "3113456789", email: "ana.fernandez@email.com", password: "Paciente123", address: "Blvd. del Valle 789", state: "Nayarit", city: "Tepic", population: "Centro", zipCode: "63000", bloodType: "B+", registrationDate: new Date().toISOString() }
        ];
        ...
        const demoAppointments = [
            { id: 1, patientId: 1, doctor: getDoctorLabel(doctors[0]), date: hoy, time: "09:00", reason: "Consulta general", status: "Agendada", createdDate: new Date().toISOString() },
            { id: 2, patientId: 1, doctor: getDoctorLabel(doctors[0]), date: mananaStr, time: "14:00", reason: "Seguimiento", status: "Agendada", createdDate: new Date().toISOString() }
        ];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    syncFromApi().catch(err => console.warn("Backend no disponible, usando datos locales:", err.message));
...
if (!patientId || !doctor || !date || !time) {
    ...
    const doctorWorksOnWeekday = doctorRecord && doctorRecord.dias_trabaja === 'L-V';
